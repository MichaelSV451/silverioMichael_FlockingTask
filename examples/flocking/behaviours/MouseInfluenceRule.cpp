...
Vector2 MouseInfluenceRule::computeForce(const std::vector<Boid*>& neighborhood, Boid* boid) {
    ...
    if (ImGui::IsMousePosValid() && io.MouseDown[0]) {
        ...
        //displacement vector is the direction from mouse to boid
        Vector2 displacement = mousePos - boid->getPosition();

        //get distance between mouse and boid
        float distance = mousePos.getDistance(mousePos, boid->getPosition());
